...
static unsigned char bufferOffset, innerHash[HASH_LENGTH], keyBuffer[BLOCK_LENGTH]; // K0 in FIPS-198a
...
inline static uint32_t ror32(uint32_t number, uint8_t bits)
	{
...
size_t sha256_write(uint8_t data)
	{
  ++byteCount;
  addUncounted(data);
	}
...
uint8_t *hmac_sha256_result(void)
	{
  uint8_t i;
  // Complete inner hash
